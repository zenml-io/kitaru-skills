...
Before cohort creation, show the exact proposed membership once and obtain confirmation together with the accepted behavior. Prefer an installed configured evaluator when it expresses that behavior and its parent is global or scoped to the tour's agent. Otherwise keep custom code local until it passes the installed evaluator test command, then register one immutable version on a parent scoped to the tour's agent. Re-read the parent and verify its returned `agent_id` before evaluation. Scope cannot be edited through evaluator update or version registration, so do not reuse a same-named parent scoped to another agent or silently fall back to a global parent.
