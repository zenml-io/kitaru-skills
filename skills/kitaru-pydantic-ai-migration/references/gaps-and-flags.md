...
| Capture policy saves sensitive prompts/tool outputs by default | `MEDIUM` | Full observability can persist user content or tool payloads. | Set `CapturePolicy(save_prompts=False, tool_capture="metadata" or None, tool_capture_overrides=...)`. |
