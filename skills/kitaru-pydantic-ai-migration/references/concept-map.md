...
| PydanticAI `requires_approval=True`, `ApprovalRequired`, or `CallDeferred` | Rewrite to `hitl_tool`, explicit flow-scope `wait_for_input`, or another documented adapter-supported pattern | `absent` until redesigned | Native approval/deferred behavior should be flagged. Do not imply it becomes a durable Kitaru wait automatically. |
